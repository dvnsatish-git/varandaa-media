...
function CalendarPanel({ region }: { region: "india" | "usa" }) {
  ...
  const festDays = [30]; // Ugadi on March 30
    ...
    <section id="spiritual" className="mb-[52px]">
      ...
      >
        ...
        <div>
          ...
          <div className="flex gap-[7px] flex-wrap">
            {["రాశిఫలాలు", "పంచాంగం", "గుడి వేళలు", "మంచి ముహూర్తాలు"].map((p) => (
              <button key={p} className="px-[13px] py-[5px] rounded-[20px] text-[11px] font-semibold border border-turmeric/30 text-turmeric bg-turmeric/[0.08] hover:bg-turmeric hover:text-night transition-all">
                {p}
              </button>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-[10px] mb-[22px]">
        {RAASHIS.map((r) => (
          <div
            key={r.en}
            className="bg-warmWhite border border-border rounded-[5px] p-[14px_12px] cursor-pointer transition-all hover:-translate-y-[3px] hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] text-center relative overflow-hidden group"
          >

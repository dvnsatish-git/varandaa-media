...
export default function OTTGrid({ onArticleClick }: OTTGridProps) {
  const { articles, loading } = useFeedCategory("ott", 6);
            ...
            >
              <div className="relative aspect-video overflow-hidden">
                ...
                <span className="absolute top-[7px] right-[7px] text-[8.5px] font-bold px-[7px] py-[2px] rounded-[2px] tracking-[0.5px] bg-teal text-white">
                  {article.sourceName.replace("Google News — ", "")}
                </span>
              </div>
              <div className="p-[10px_12px]">
                ...
                <p className="text-[9px] text-white/30 mt-1">{timeAgo(article.publishedAt)} · {article.tags.join(", ")}</p>
              </div>
            </div>
          ))
          : OTT_DATA.map((item, i) => (
            ...
            >
              <div className="relative aspect-video overflow-hidden">
                ...
                <span
                  className="absolute top-[7px] right-[7px] text-[8.5px] font-bold px-[7px] py-[2px] rounded-[2px] tracking-[0.5px]"
